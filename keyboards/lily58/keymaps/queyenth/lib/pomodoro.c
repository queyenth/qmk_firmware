...
void draw_session_icon(uint8_t x, uint8_t y, uint8_t width)
{
  ...
  for (size_t i = 0; i < 32; i++) {
    ...
    for (size_t j = 0; j < 8; j++) {
      uint8_t new_y = y + j + lineN * 8;
      oled_write_pixel(new_x, new_y, (bool)(sessionByte & (1 << j)));
    }
  }
}
...
void draw_by_animation(uint8_t fill, bool animation)
{
  ...
  while (fill_from <= fill_to) {
    ...
    for (uint8_t b = 0; b < bytesToChange; b+=2) {
      // Would it be cheaper to read the word and then << 8? Dunno.
