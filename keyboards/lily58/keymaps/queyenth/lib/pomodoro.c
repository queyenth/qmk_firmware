...
void draw_number(uint8_t x, uint8_t y, char number)
{
  ...
  for (size_t i = 0; i < POMODORO_NUMBER_FONT_HEIGHT; i++) {
    unsigned char bitN = 1 << i;
    for (size_t j = 0; j < POMODORO_NUMBER_FONT_WIDTH; j++) {
      if (pgm_read_byte(numbersFont+number+j) & bitN) {
	oled_write_pixel(j+x, i+y, true);
      } else {
	oled_write_pixel(j+x, i+y, false);
      }
    }
  }
}
...
void draw_session_icon(uint8_t x, uint8_t y)
{
  for (size_t i = 0; i < 8; i++) {
	...
	new_x -= 16;
      ...
      if (pgm_read_byte(sessionLogo+j) & bitN) {
	oled_write_pixel(new_x, new_y, true);
      } else {
	oled_write_pixel(new_x, new_y, false);
      }
    }
  }
}
...
uint8_t prev_fill = 0;
void draw_by_animation(uint8_t fill, char animation)
{
  ...
  while (fill_from <= fill_to) {
    ...
    bytesToChange -= f;
    for (uint8_t b = 0; b < bytesToChange; b+=2) {
...
void update_clock(void)
{
  ...
  if (pomodoroStatus == work) {
    ...
    fill_parts = POMODORO_STEPS - (double)(POMODORO_STEPS * elapsedPomodoro / POMODORO_REST); // we fill the rest in another order. Which means we should clear some pixels.
  }
...
void render_pomodoro(void) {
  ...
  draw_session_icon(0, 38);
  draw_number_string(20, 42, pomodoroSessionString);
  draw_number_string(10, 56, pomodoroTimerString);
  // For icon and string above we need to advance 4 pages
...
void update_pomodoro(void) {
  ...
  if (lastTimerUpdateTimer == 0 || timer_elapsed32(lastTimerUpdateTimer) >= POMODORO_UPDATE_TIMER) {
    uint32_t pomodoroLeft = (pomodoroTimer - elapsedSinceStart);
    uint8_t minutes = pomodoroLeft / POMODORO_MINUTE;
    if (minutes > MAX(pomodoroWork, pomodoroRest)) {
