...
static bool on_left_hand(keypos_t pos) {
  return pos.row < (MATRIX_ROWS - 3) / 2;
}

bool achordion_opposite_hands(const keyrecord_t* tap_hold_record,
                              const keyrecord_t* other_record) {
  const keypos_t tap_hold_keypos = tap_hold_record->event.key;
  const keypos_t other_record_keypos = other_record->event.key;
  // if one of the keys is the thumb cluster
  // consider it the opposite hand
  if (tap_hold_keypos.col >= (MATRIX_COLS - 2)
      || other_record_keypos.col >= (MATRIX_COLS - 2)) {
    return true;
  }

  return on_left_hand(tap_hold_keypos) !=
         on_left_hand(other_record_keypos);
}
